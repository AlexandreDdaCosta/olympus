...
from olympus import ARGON2_CONFIG, MONGODB_SERVICE, RESTAPI_RUN_USERNAME
...
def insert_update_restapi_user(username, password, defined_routes=None):
    # Hashing parameters based on OWASP cheat sheet recommendations
    # (as of March 2022)
    argon2Hasher = argon2.PasswordHasher(
        memory_cost=ARGON2_CONFIG['memory_cost'],
        parallelism=ARGON2_CONFIG['parallelism'],
        salt_len=ARGON2_CONFIG['salt_bytes'],
...
def purge_users(valid_users=None):
    ...
    for user in users:
        if (user['user'] != MONGO_ADMIN_USERNAME and
                user['user'] not in valid_users):
...
def update_object(database, collection, query, keyvals, user=USER):
    connector = mongodb.Connection(user)
    coll = connector.connect(database, collection)
    coll.update_one(query, {"$set": keyvals})
    return True
...
def user(username, password, admin=False, roles=None):
    ...
    user_entry = database['system.users'].find_one({"user": username},
                                                   {'_id': 0, 'user': 1})
