    ...
    if (server is not None and server != exclude_server):
        ...
        if 'frontend' in services:
            ...
            if os.path.isfile(frontend_credential_file):
                ...
                cmd = "perl -e 'open my $in, \"" + frontend_credential_file  + "\" or die \"$!\"; $/ = undef; my $all = <$in>; close $in; $all =~ s/(\\'PASSWORD\\'\\:\s*\\').*?(\\')/$1" + passphrase + "$2/; open my $out, \">$file\" or die \"$!\"; print $out $all; close $out;'"
                p = subprocess.check_call(cmd,shell=True)
