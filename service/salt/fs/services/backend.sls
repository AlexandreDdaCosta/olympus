...
frontend_db_user:
  postgres_user.present:
    ...
    - password: 'md5{MD5OF({{ pillar['persistent_key']['frontend_db_key'] }})}'

frontend_db_user_pwd_reset:
  cmd.run:
    - name: sudo -u postgres psql -c "ALTER USER {{ pillar['frontend-user'] }} ENCRYPTED PASSWORD '{{ pillar['persistent_key']['frontend_db_key'] }}';"
