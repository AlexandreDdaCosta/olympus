...
#from migrations.core.jobloop import JOB_CHECKIN_GRACE_SECONDS, JOB_CHECKIN_INTERVAL_SECONDS
...
def job_tile():
    ...
    results = Job.objects.filter(last_jobstatus__create_date__range=(seven_days_ago,tomorrow)).prefetch_related('last_jobstatus')
    ...
    if results:
        ...
        for job in results:
            if job.last_jobstatus:
                ...
                if last_status == 'inprogress' and job.last_jobstatus.create_date < timezone.now()-timedelta(seconds=JOB_CHECKIN_INTERVAL_SECONDS+JOB_CHECKIN_GRACE_SECONDS):
                    last_status = 'stuck'
                if last_status not in jobs['status']:
                    jobs['status'][last_status] = 0
                jobs['status'][last_status] = jobs['status'][last_status] + 1
    '''
    return { 'checkdate': timezone.now(), 'jobs': jobs, 'seven_days_ago': str(seven_days_ago), 'tomorrow': str(tomorrow) }
...
def minion_tile():
    ...
    '''
    try:
        job = Job.objects.filter(last_jobstatus_type__name='completed',type__codename='monitor_minions').latest('execution_date')
    except Job.DoesNotExist:
        job = None
    if job and job is not None:
        jobstatus = JobStatus.objects.filter(status__name='completed',job__id=job.id)
        ...
        pass
    '''
    return { 'checkdate': checkdate, 'jobstatus': jobstatus, 'minions': minions }
