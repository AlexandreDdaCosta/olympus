...
QUARTERLY_YEAR_LIST = range(QUARTERLY_FIRST_YEAR,
...
class InitForm4Indices(data.Initializer):
    ...
    def __init__(self, username=USER, **kwargs):
        super(InitForm4Indices, self).__init__(FORM4_INDEX_COLLECTION_NAME,
                                               username,
                                               **kwargs)

    def populate_collections(self):  # noqa: C901
        self.prepare()
        if self.verbose:
            print('Initializing Form4 collection procedure.')

        # The edgar module retrieves based on a starting year.
        # Read existing collection, looking for last worked year,
        ...
        download_directory = ('/tmp/form4_indices_' +
        ...
        for year in collection_range:
                ...
                if os.path.isfile(filename):
                    with open(filename) as f:
                        for line in f:
                            ...
                            row['file'] = pieces[4]
                            row['file'] = re.sub(r'^.*\/(.*)\.txt',
                                                 r'\g<1>',
                                                 pieces[4])
                            row['form'] = str(pieces[2])
                            row['year'] = year
                            downloaded_entries.append(str(row['issuerCik']) +
                                                      '_' +
                                                      row['file'] +
                                                      '_' +
                                                      row['form'])
            ...
            if year_document_count > 0:
                # Execution check: If number of existing documents match
                # relevant rows in download, assume no changes and skip
                if len(downloaded_entries) == year_document_count:
                    if self.verbose:
                        print('Download document count matches count of ' +
                              'existing documents; bypassing ' +
                              str(year) + '.')
                    continue
                # Differentiate collections, add missing entries to
                # existing collection
                if self.verbose:
                    print('Differentating new download and existing ' +
                          'collection for ' +
                          str(year) + '.')
                existing_entries = []
                for entry in collection.find({'year': year},
                                             {'issuerCik': 1,
                                              'file': 1,
                                              'form': 1,
                                              '_id': 0}):
                    existing_entries.append(str(entry['issuerCik']) +
                                            '_' +
                                            entry['file'] +
                                            '_' +
                                            entry['form'])
                ...
                if len(missing_entries) == 0:
                    ...
                    for entry in missing_entries:
                        row = {}
                        row['issuerCik'], row['file'], row['form'] = \
                            entry.split('_')
            ...
            if create_indices is True:
                if self.verbose:
                    print('Creating indices for ' +
                          FORM4_INDEX_COLLECTION_NAME)
                collection.create_index([('issuerCik', pymongo.ASCENDING)],
                                        name=(FORM4_INDEX_COLLECTION_NAME +
                                              '_issuerCik_' +
                                              INDEX_SUFFIX),
                                        unique=False)
...
class Form4(data.Connection):
    ...
    def populate_indexed_forms(self, **kwargs):  # noqa: C901

        # Gather detailed Form4 records based on downloaded EDGAR indices
        self.epoch_time = int(time.time())
        schema_directory = re.sub(r'(.*\/).*?$',
                                  r'\1',
                                  os.path.dirname(
                                      os.path.realpath(__file__))) + 'schema/'
        self.form4_schema = xmlschema.XMLSchema(schema_directory +
                                                'ownership4Document.xsd.xml')
        self.form4a_schema = xmlschema.XMLSchema(schema_directory +
                                                 'ownership4ADocument.xsd.xml')
        ...
        self.collection_issuer.create_index([('issuerCik', pymongo.ASCENDING)],
                                            name=(self.ISSUER_COLLECTION_NAME +
                                                  '_issuerCik_' +
                                                  INDEX_SUFFIX),
                                            unique=True)
        self.collection_reporting_owner = \
            self.db[self.REPORTING_OWNER_COLLECTION_NAME]
        self.collection_reporting_owner.create_index(
            [('rptOwnerCik', pymongo.ASCENDING)],
            name=(self.REPORTING_OWNER_COLLECTION_NAME +
                  '_rptOwnerCik_' +
                  INDEX_SUFFIX),
            unique=True)
        self.collection_submissions = \
            self.db[self.FORM4_SUBMISSIONS_COLLECTION_NAME]
        self.collection_submissions.create_index(
            [('rptOwnerCik', pymongo.ASCENDING)],
            name=(self.FORM4_SUBMISSIONS_COLLECTION_NAME +
                  '_rptOwnerCik_' +
                  INDEX_SUFFIX),
            unique=True)
        ...
        if cik_file is not None:
            ...
            records = []
            for record in self.collection_submissions.find(
                {'issuerCik': int(issuerCik),
                 'processing': {'$exists': False},
                 'year': int(year)}):
                records.append(record)
            if len(records) == 0:
                raise Exception('Specified cik/file combination not found' +
                                'in indices: ' +
                                cik_file)
            elif len(records) > 1:
                raise Exception('Specified cik/file combination found ' +
                                str(len(records)) +
                                ' times in indices: ' +
                                cik_file)
            ...
            while True:
                ...
                if len(records) == 0:
                    if self.verbose and not self.done:
                        print('No unprocessed index entries found for ' +
                              str(year) +
                              '; ending.')
                    break
                self._get_write_forms(year, records)
    ...
    def _fix_xml_content(self, xml_content, e):
        ...
        if e.reason == ("invalid datetime for formats " +
                        "('%Y-%m-%d', '-%Y-%m-%d')."):
            path = e.path
    ...
    def _get_write_forms(self, year, records):  # noqa: C901
        download_directory = ('/tmp/form4_submissions_' +
                              str(os.getpid()) +
                              '_' +
                              str(self.epoch_time) +
                              '/')
        ...
        for record in records:
            try:
                ...
                while xml_validate is True:
                    try:
                        if record['form'] == '4/A':
                            xmlschema.validate(xml_content,
                                               schema=self.form4a_schema)
                        else:
                            xmlschema.validate(xml_content,
                                               schema=self.form4_schema)
                        ...
                        if xml_content is None:
                            if self.verbose:
                                print('\n\nhttps://www.sec.gov/' +
                                      ...
                                      record['file'] +
                                      '.txt\n')
                                print('Gracefully bypassing record ' +
                                      str(record['_id']) +
                ...
                if (self.collection_issuer
                        .find({'issuerCik': record['issuerCik']})
                        .limit(1)
                        .count() == 0):
                    self.collection_issuer.update(
                        {'issuerCik': record['issuerCik']},
                        {'issuerCik': record['issuerCik']},
                        upsert=True)
                    self.collection_issuer.update(
                        {'issuerCik': record['issuerCik']},
                        {'$set': {'issuer': {}, 'history': {}}})
                submission_date = \
                    self._convert_period_of_report(
                        data['ownershipDocument']['periodOfReport'])
                issuer_record = {}
                for issuer_record in self.collection_issuer.find(
                        {'issuerCik': record['issuerCik']}):
                    break
                new_dict = {}
                new_dict['periodOfReport'] = submission_date
                new_dict['issuerName'] = \
                    data['ownershipDocument']['issuer']['issuerName'].upper()
                new_dict['issuerTradingSymbol'] = \
                    (data['ownershipDocument']['issuer']['issuerTradingSymbol']
                     .upper())
                if len(issuer_record['issuer']) == 0:
                    self.collection_issuer.update(
                        {'_id': issuer_record['_id']},
                        {'$set': {'issuer': new_dict}})
                elif (issuer_record['issuer']['periodOfReport'] <
                        submission_date):
                    if (issuer_record['issuer']['issuerName'] !=
                            new_dict['issuerName'] or
                            issuer_record['issuer']['issuerTradingSymbol'] !=
                            new_dict['issuerTradingSymbol']):
                        historic_dict = {}
                        historic_dict['issuerName'] = \
                            issuer_record['issuer']['issuerName']
                        historic_dict['issuerTradingSymbol'] = \
                            issuer_record['issuer']['issuerTradingSymbol']
                        self.collection_issuer.update(
                            {'_id': issuer_record['_id']},
                            {'$set': {'issuer':
                                      ...
                                      'history.' +
                                      str(issuer_record
                                          ['issuer']['periodOfReport']):
                                      historic_dict}})

                # reportingOwner document + submission data
                owner_cik = []
                if type(data['ownershipDocument']['reportingOwner']) is list:
                    for reportingOwner in (data['ownershipDocument']
                                           ['reportingOwner']):
                        rptOwnerCik = self._reporting_owner_handler(
                            data,
                            record,
                            reportingOwner)
                ...
                self.collection_index.update(
                    {'issuerCik': record['issuerCik'],
                     'file': record['file']},
                    {'$set': {'rptOwnerCik': owner_cik},
                     '$unset': {'failure': 1,
                                'processing': 1,
                                'pid': 1}})
                if self.verbose:
    ...
    def _reporting_owner_handler(self,  # noqa: C901
                                 data,
                                 record,
        ...
        rptOwnerCik = int(reporting_owner_dict['reportingOwnerId']
                          ['rptOwnerCik'])
        if (self.collection_submissions.find(
                {'rptOwnerCik': rptOwnerCik})
                .limit(1)
                .count()) == 0:
            self.collection_submissions.update(
                {'rptOwnerCik': rptOwnerCik},
                {'rptOwnerCik': rptOwnerCik,
                 'issuerCik': {}},
                upsert=True)
        ...
        if 'reportingOwnerRelationship' in reporting_owner_dict:
            relationship_keys = ('isDirector',
                                 'isOfficer',
                                 'isTenPercentOwner')
            for key in relationship_keys:
                if (key in
                        reporting_owner_dict['reportingOwnerRelationship'] and
                        (str(reporting_owner_dict
                             ['reportingOwnerRelationship']
                             [key]
                             .strip())) == '1'):
                    reportingOwner[key] = True
            if ('officerTitle' in
                    reporting_owner_dict['reportingOwnerRelationship']):
                officer_title = (reporting_owner_dict
                                 ['reportingOwnerRelationship']
                                 ['officerTitle']
                                 .strip())
                if officer_title:
        ...
        if ('nonDerivativeTable' in data['ownershipDocument'] and
            ('nonDerivativeTransaction' in
             (data['ownershipDocument']['nonDerivativeTable'])) and
            (data['ownershipDocument']
             ['nonDerivativeTable']
             ['nonDerivativeTransaction'])):
            print('nonDerivativeTransaction')
            if (type(data['ownershipDocument']
                     ['nonDerivativeTable']
                     ['nonDerivativeTransaction'])) is not list:
                nonDerivativeTransaction.append(
                ...
                print('LIST')
                for transaction in (data['ownershipDocument']
                                    ['nonDerivativeTable']
                                    ['nonDerivativeTransaction']):
                    nonDerivativeTransaction.append(
                        self._extract_transaction(transaction))

        derivativeTransaction = []
        if ('derivativeTable' in data['ownershipDocument'] and
            ('derivativeTransaction' in
             data['ownershipDocument']['derivativeTable']) and
            (data['ownershipDocument']
             ['derivativeTable']
             ['derivativeTransaction'])):
            ...
            if (type(data['ownershipDocument']
                     ['derivativeTable']
                     ['derivativeTransaction'])) is not list:
                derivativeTransaction.append(
                    self._extract_transaction(
                        (data['ownershipDocument']
                         ['derivativeTable']
                         ['derivativeTransaction'])))
        ...
        if self.collection_reporting_owner.find(
                {'rptOwnerCik': rptOwnerCik}).limit(1).count() == 0:
            self.collection_reporting_owner.update(
                {'rptOwnerCik': rptOwnerCik},
                {'rptOwnerCik': rptOwnerCik},
                upsert=True)
            self.collection_reporting_owner.update(
                {'rptOwnerCik': rptOwnerCik},
                {'$set': {'reportingOwner': {}, 'history': {}}})
        submission_date = self._convert_period_of_report(
            data['ownershipDocument']['periodOfReport'])
        reporting_owner_record = {}
        for reporting_owner_record in self.collection_reporting_owner.find(
                {'rptOwnerCik': rptOwnerCik}):
            break
        new_dict = {}
        new_dict['periodOfReport'] = submission_date
        if reporting_owner is not None:
            new_dict['rptOwnerName'] = \
                reporting_owner['reportingOwnerId']['rptOwnerName'].upper()
        else:
            new_dict['rptOwnerName'] = (data['ownershipDocument']
                                        ['reportingOwner']
                                        ['reportingOwnerId']
                                        ['rptOwnerName']
                                        .upper())
        new_dict['reportingOwnerAddress'] = ''
        address_keys = ('rptOwnerStreet1',
                        'rptOwnerStreet2',
                        'rptOwnerCity',
                        'rptOwnerState',
                        'rptOwnerZipCode')
        for address_key in address_keys:
            if (reporting_owner is not None and
                    'reportingOwnerAddress' in reporting_owner):
                if (
                    ...
                    new_dict['reportingOwnerAddress'] += \
                        (reporting_owner['reportingOwnerAddress']
                         [address_key]
                         .strip() +
                         ' ')
            elif ('reportingOwnerAddress' in
                    data['ownershipDocument']['reportingOwner']):
                if (
                       address_key in (data
                                       ['ownershipDocument']
                                       ['reportingOwner']
                    ...
                    new_dict['reportingOwnerAddress'] += \
                        (data
                         ...
                         ' ')
        new_dict['reportingOwnerAddress'] = \
            new_dict['reportingOwnerAddress'].rstrip()
        if len(reporting_owner_record['reportingOwner']) == 0:
            self.collection_reporting_owner.update(
                {'_id': reporting_owner_record['_id']},
                {'$set': {'reportingOwner': new_dict}})
        elif (reporting_owner_record['reportingOwner']['periodOfReport'] <
              submission_date):
            if (
                   (reporting_owner_record['reportingOwner']['rptOwnerName'] !=
                    new_dict['rptOwnerName'])
                   or ('reportingOwnerAddress' in
                       reporting_owner_record['reportingOwner'] and
                       (reporting_owner_record
                        ['reportingOwner']
                        ['reportingOwnerAddress']) !=
                       new_dict['reportingOwnerAddress'])
                   or ('reportingOwnerAddress' not in
                       reporting_owner_record['reportingOwner'])
               ):
                historic_dict = {}
                historic_dict['rptOwnerName'] = \
                    reporting_owner_record['reportingOwner']['rptOwnerName']
                if ('reportingOwnerAddress' in
                        reporting_owner_record['reportingOwner']):
                    historic_dict['reportingOwnerAddress'] = \
                        (reporting_owner_record
                         ['reportingOwner']
                         ['reportingOwnerAddress'])
                self.collection_reporting_owner.update(
    ...
    def _revert_unlock_slice(self, records, id, error):
        ...
        for record in records:
            bulk.find(
                {'_id': record['_id'],
                 'processing': self.epoch_time,
                 'pid': os.getpid()}).update(
                     {'$unset': {'failure': 1, 'processing': 1, 'pid': 1}})
            if self.verbose:
    ...
    def _select_lock_slice(self, year):
        ...
        bulk = self.collection_index.initialize_ordered_bulk_op()
        for record in self.collection_index.find(
            {'rptOwnerCik': {'$exists': False},
             'failure': {'$exists': False},
             'processing': {'$exists': False},
             'year': int(year)}).limit(block_size):
            self.max_record_count += 1
            records.append(record)
            bulk.find(
                {'_id': record['_id']}).update(
                    {'$set': {'processing': self.epoch_time,
                              'pid': os.getpid()}})
        if len(records) > 0:
            bulk.execute()
        return records
