...
class TestPrice(testing.Test):
    ...
    def test_adjustments(self):
        ...
        regen_adjustments = adjustments.adjustments(TEST_SYMBOL_DIVSPLIT,regen=True)
    ...
    def test_daily(self):
        ...
        with self.assertRaises(Exception):
            daily.quote(TEST_SYMBOL_DIVSPLIT,start_date=dt(2022, 2, 29, 0, 0, 0).replace(tzinfo=tz.gettz(TIMEZONE)))
        with self.assertRaises(Exception):
            daily.quote(TEST_SYMBOL_DIVSPLIT,end_date=dt(2022, 2, 29, 0, 0, 0).replace(tzinfo=tz.gettz(TIMEZONE)))
        with self.assertRaises(Exception):
        ...
        with self.assertRaises(Exception):
            daily.quote(TEST_SYMBOL_DIVSPLIT,start_date=today,end_date=a_while_ago)
        with self.assertRaises(Exception):
            daily.quote(TEST_SYMBOL_DIVSPLIT,period='BADPERIOD')
        with self.assertRaises(Exception):
            daily.quote(TEST_SYMBOL_DIVSPLIT,period='1Y',start_date=a_while_ago)
        with self.assertRaises(Exception):
            daily.quote(TEST_SYMBOL_DIVSPLIT,period='1Y',end_date=a_while_ago)
        quotes = daily.quote(TEST_SYMBOL_DIVSPLIT,start_date=a_while_ago,end_date=today)
        ...
        self.print('Remove the last price record by date, then get the quote again. Check that the record was restored.')
        quotes = daily.quote(TEST_SYMBOL_DIVSPLIT,regen=True)
    ...
    def test_latest(self):
        ...
        self.print('Mixed request including (1) Valid symbol, (2) Valid but incorrectly cased symbol, and (3) Unknown symbol.')
        result = latest.quote([TEST_SYMBOL_NODIVSPLIT.lower(),TEST_SYMBOL_DIVSPLIT,TEST_SYMBOL_FAKE.lower()])
        quoteTimeLong = int(time.time() * 1000)
    ...
    def test_intraday(self):
        ...
        self.print('The next two series use the default values for period and frequency.')
        ...
        while quote is not None:
            if last_quote_time is not None and quote['Date'].day == last_quote_time.day:
                ...
                if last_quote_time.hour >= 9 and last_quote_time.minute >=30 and last_quote_time.hour < 16:
                    self.assertEqual(intraday.DEFAULT_INTRADAY_FREQUENCY, int((quote['Date'] - last_quote_time).total_seconds()/60))
            last_quote_time = quote['Date']
            quote = quotes.next(return_raw_data=True)
        quotes = intraday.quote(TEST_SYMBOL_DIV,need_extended_hours_data=False)
        quote = quotes.next()
        last_quote_time = None
        while quote is not None:
            ...
            if last_quote_time is not None and (quote.date.day > last_quote_time.day or quote.date.month > last_quote_time.month):
                yesterday_close_time_string = (last_quote_time + timedelta(minutes=intraday.DEFAULT_INTRADAY_FREQUENCY)).strftime("%H:%M:%S")
        ...
        for period in intraday.VALID_INTRADAY_PERIODS:
            with self.assertRaises(Exception):
                ...
                intraday.quote(TEST_SYMBOL_DIVSPLIT,period=period,start_date=two_days_ago)
            for frequency in intraday.VALID_INTRADAY_FREQUENCIES:
                ...
                oldest_available_date = intraday.oldest_available_date(frequency)
                error_date = oldest_available_date + timedelta(days=period-2)
                with self.assertRaises(Exception):
                    intraday.quote(TEST_SYMBOL_NODIVSPLIT,need_extended_hours_data=False,period=period,frequency=frequency,end_date=error_date)
                # The next series is limited to regular trading hours to ensure there are no time periods with no data
                quotes = intraday.quote(TEST_SYMBOL_DIVSPLIT,period=period,frequency=frequency,need_extended_hours_data=False)
                last_quote_time = None
                quote = quotes.next()
                while quote is not None:
        ...
        with self.assertRaises(Exception):
            intraday.quote(TEST_SYMBOL_SPLIT,end_date=two_days_ago,start_date=yesterday)
        self.print('Checks for dates that are too early when combined with a frequency and for proper date ranges.')
        for frequency in intraday.VALID_INTRADAY_FREQUENCIES:
            ...
            with self.assertRaises(Exception):
                intraday.quote(TEST_SYMBOL_SPLIT,frequency=frequency,start_date=too_early)
            with self.assertRaises(Exception):
                intraday.quote(TEST_SYMBOL_SPLIT,frequency=frequency,end_date=too_early)
            quotes = intraday.quote(TEST_SYMBOL_SPLIT,frequency=frequency,start_date=earliest_available_date)
            ...
            quotes = intraday.quote(TEST_SYMBOL_SPLIT,frequency=frequency,end_date=half_way_end_date)
            last_quote = quotes.last()
