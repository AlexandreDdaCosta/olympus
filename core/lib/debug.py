...
class DebuggerArgs():
    ...
    def add_arguments(self):
        ...
        parser.add_argument("-F","--to_file",action='store_true',help="Sends debugging output to a prenamed output file; disables all processing pauses")
    ...
    def __init__(self,debug_args=None):
        ...
        if self.args.to_file:
            if not hasattr(self.args, 'filehandle'):
                file_name = '/tmp/debug.' + os.path.basename(sys.argv[0]) + '.' + str(time.time())
                self.args.filehandle = open(file_name,"a")
                os.chmod(file_name, 0o0440)
                subprocess.run(["/bin/bash -c 'read -n 1 -s -p \"Output to file " + file_name + "; press any key to continue ...\"'"], stdout=subprocess.PIPE, shell=True)
                print()
    ...
    def debug(self,debug_level=DEFAULT_DEBUG_LEVEL,message='Debug message left blank.',data=None,**kwargs):
        ...
        if debug_level <= self.args.debug_level:
            if self.args.debug_level == 1:
                ...
                self._print_debug_output("{message} [{sourcefile}, {lineno}]".format(message=message, sourcefile=sourcefile, lineno=caller_frameinfo.lineno))
            if data is not None and self.args.debug_level >= 2:
                if isinstance(data, object) and hasattr(data, '__dict__'):
                    self._print_debug_output(json.dumps(vars(data), default=str, indent=4, sort_keys=True))
                else:
                    self._print_debug_output(json.dumps(data, default=str, indent=4, sort_keys=True))
                if not self.args.to_file:
                    no_pause = kwargs.get('no_pause',False)
                    if debug_level > 1 and debug_level == self.args.debug_level and no_pause is False:
                        result = subprocess.run(["/bin/bash -c 'read -n 1 -s -p \"<<<< (q=quit) \" input; echo $input'"], stdout=subprocess.PIPE, shell=True)
