...
class Connection(redis.Connection):
    ...
    def _token(self, test_access=False):  # noqa: F403
        ...
        if (hasattr(self, 'access_token') and
                hasattr(self, 'access_token_expiration') and
                int(self.access_token_expiration) < int(time.time()) + 30):
            return self.access_token

        # 2. Current and valid access token in redis?

        redis_client = self.client()
        redis_stored_tokens = redis_client.hgetall('user:' +
                                                   self.username +
                                                   ':restapi:token')
        if redis_stored_tokens is not None:
            if ('access_token_expiration' in redis_stored_tokens and
                    (int(redis_stored_tokens['access_token_expiration']) >
                     int(time.time()) + 30)):
                if ((test_access is True) and
                    (not self._test_token(
                        redis_stored_tokens['access_token']))):
                    pass
                else:
                    self.access_token = redis_stored_tokens['access_token']
                    self.access_token_expiration = \
                        redis_stored_tokens['access_token_expiration']
                    return self.access_token

        # 3. Current and valid refresh token in redis?

            elif ('refresh_token_expiration' in redis_stored_tokens and
                    (int(redis_stored_tokens['refresh_token_expiration']) >
                     int(time.time()) + 30)):
                ...
                if response.status_code == 200:
                    ...
                    redis_client.hset('user:' +
                                      ...
                                      content['access_token_expiration'])
                    redis_client.hset('user:' +
                                      self.username +
                                      ':restapi:token',
                                      'access_token',
                    ...
                    redis_client.hset('user:' +
                                      ...
                                      content['refresh_token_expiration'])
                    redis_client.hset('user:' +
                                      self.username +
                                      ':restapi:token',
                                      'refresh_token',
...
class RestAPIError(Exception):
    ...
    def __str__(self):
        ...
        if self.response is None:
            message = (message +
                       'No response to attempted call of backend API server.')
        else:
            message = (message +
                       ' Status code ' +
